    ...
    >
      <template #cell(id)="data">
        <span v-if="data.item.name === undefined">
          {{ data.item.id }}
          <b-button :href="mailToken(data.item.id)"><font-awesome-icon :icon="['fas', 'envelope']" /> {{$t('Mail')}}</b-button>
          <b-button @click="copyToClipboard(data.item.id)"><font-awesome-icon :icon="['fas', 'copy']" /> {{$t('Copy')}}</b-button>
        </span>
        <span v-else>
